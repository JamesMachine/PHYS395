...
recursive subroutine gaussj(n, A, B)
	...
	! pivot indices
	integer row(n), col(n), pivot(2) !1d array
	...
	! find location of maximal value of entire array
	pivot = maxloc(abs(A))

	! swap rows/columns ! swaping only index by modifying 1D array
	row([1,pivot(1)]) = row([pivot(1),1])
	col([1,pivot(2)]) = col([pivot(2),1])

  !in Python
  ! a, b = b, a
  ! row(1), row(pivot(1)) = row(pivot(1)), row(1)

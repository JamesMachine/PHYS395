...
write(*,*)
write(*,*) 'Approximating 100 sample points using 100 terms (q5) -------------------'
!plotting samples
open(1, file = 'output5_1_2.csv', status='unknown')
do i=1,n
  y(i)=f(x(i))
  write(1,*)  x(i), y(i)
end do
...
open(1, file = 'output5_2_2.csv', status='unknown')
do i=1,k
...
open(1, file = 'output5_3_2.csv', status='unknown')
...
open(1, file = 'output5_4_2.csv', status='unknown')
do i=1,k
...
! plotting expected curve
open(1, file = 'output5_5_2.csv', status='unknown')
do i=1,k
...
open(1, file = 'output5_6_2.csv', status='unknown')
...
!plotting error taking abolute value of expected values and approximated values
open(1, file = 'output5_7_2.csv', status='unknown')
do i=1,k
  error(i) = abs(yexpect(i)-yapprox(i))
  write(1,*) xexpect(i), error(i)
end do

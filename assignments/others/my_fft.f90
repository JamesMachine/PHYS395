...
do k = 1,N/2
	fft(k) = (0,1)*(2.0*pi)*(k-1) * fft(k)		! positive frequencies;<-- probably derivative
	fft(n+1-k) = -(0,1)*(2.0*pi)*k * fft(n+1-k)	! negative frequensies; <-- probably derivative for negative
end do
...
do i = 1,N
	write (*,*) x(i), abs(in(i)), abs(fft(i)), real(out(i)/N)
end do

...
! compile with: gfortran -O3 -fdefault-real-8 -fopenmp double_pendulum_flip.f90 -lcfitsio
...
thrange1 = (/ hold(1), hold(2) /)
thrange2 = (/ hold(3), hold(4) /)
...
do i = 1,nth
	...
	do j = 1,nth/2
		...
		data(1,i,j) = integrate(th1, th2, 1000.0*sqrt(l/g), dt)/100.0
		write(*,*) i,j, data(1,i,j)
		data(1,nth-(i-1),nth-(j-1)) = data(1,i,j)
	end do
end do

...
do varia = 0,5
	fa = integrate(0.0, 4.0, 1e-3, varia)
end do
...
subroutine evalf(u, dudt, num)
        ...
        dudt(1) = 1.0; dudt(2) = u(3); dudt(3) = 2.0*(V(x) - (0.5 + num*2.0))*u(2)
        end associate
end subroutine evalf

! 10th order implicit Gauss-Legendre integrator
subroutine gl10(y, dt, num)
        integer, parameter :: s = 5, n = 3
        real y(n), g(n,s), dt; integer i, k, num
...
function integrate(phi, t, dt, num); intent(in) phi, t, dt, num
	real phi, t, dt, integrate
	real u(3), E0; integer i, n, num
	...
	do i = 1,n
		call gl10(u, dt, num); if (abs(u(1)) > 100.0) exit
		if (mod(i, 100)==0) write (*,'(4g24.16)') u(1), (u(2))**2, u(3)
	end do

	call gl10(u,t-n*dt, num)

...
! compile with: gfortran -O3 -fdefault-real-8 prob2.f90 -o a && ./a
...
program simple_harmonic
    ...
    real a, guess,x, dx, neg , fa, fb, n, E1,E2,E3;
    integer i, j,alpha
    ...
    do i=1,5
    E1_range(i)=0.2+2.0*(i-1);
    E2_range(i)=0.8+2.0*(i-1);
    end do
    ...
    do j=1,10;
        ...
        ! fb=integrate(a, guess, x, dx,(j+1),E3) 
         fa=integrate(a, guess, neg*x, neg*dx,3,E3) 
         fb=integrate(a, guess, x, dx,4,E3) 
    end do 

    contains
    
    !Bisection_function
    function bisection(a,b); !intent(in) a,b,E
        real a,b,E,c,fa,fb,fc, bisection
            fa = f(a)
            fb = f(b)    
    ...
    function f(E); intent(in) E
        real f, E
        f = integrate(a, guess, x, dx,2,E) 
    end function

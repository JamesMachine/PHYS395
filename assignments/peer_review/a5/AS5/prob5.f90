...
program problem4
    ...
    write(*,*) "--------------------------------------------------"
    write(*,*) "Eigenvalue of the energy for anharmionic potential"
    write(*,*) "--------------------------------------------------"
    call lsolve()
    
    ! Evolve until 10th energy eigen state and save the data
    open(unit=5,file="data_q5_1.dat")
    do j = 1,nn
        write (5,'(4g24.16)')
    ...
    subroutine lsolve()
        real A(n,n), WR(n), WI(n), VL(n,n), VR(n, n), work(nn*n)
        integer i, ind(nn), status
        ...
        status = 0; select case (kind(A))
            case(4); call sgeev('V', 'V', n, A, n, WR, WI, VL, n, VR, n, work, n*nn, status)
            case(8); call dgeev('V', 'V', n, A, n, WR, WI, VL, n, VR, n, work, n*nn, status)
            case default; call abort
        end select
    ...
    subroutine dump(eigen_vector)
        ...
        do i = 1,n
            write (5,'(4g24.16)') x(i), eigen_vector(i)
        end do
        ! close(4)
        ! write (1,*) ""; write (1,*) ""
    end subroutine

...
program a2code
  ...
  integer, parameter :: n = 8 !(n=0, 1, 2, 3, 4 ,5, 6, 7)
  integer,parameter :: numpts = 1000
  integer,parameter :: m = 9130 !number of original data points
  real, parameter :: pi = 3.141592653589793238462643383279502884197169399375105821Q0


  real x, y, yexpect, A(m,n), B(n), U(m) !this is professor way to use do while loop
  ...
  open(1,file='output4.dat',status='unknown')
  ...
  write(*,*) 'Best fit parameter is:',U(1:n)
...
contains
  ...
  subroutine dgelss_solve(m,n,A,U)
    integer:: m,n,rank,status
    real::A(m,n),U(m),S(m),W(6*m)

    ! initialize status to all clear
    status = 0

    ! call appropriate version of xgelss
    select case (kind(A))
      case(4); call sgelss(m,n,1,A,m,U,m,S,m,rank,W,6*m,status)
      case(8); call dgelss(m,n,1,A,m,U,m,S,m,rank,W,6*m,status)
      case default; call abort
    end select

    ! abort at first sign of trouble
    if (status /= 0) stop "singular matrix in dgelss_solve()"
  end subroutine
